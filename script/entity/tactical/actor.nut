this.actor <- this.inherit("scripts/entity/tactical/entity", {
	...
	function getTooltip( _targetedWithSkill = null )
	{
		...
		else
		{
			...
			{
				local回合后ToGo = this.Tactical.TurnSequenceBar.getTurnsUntilActive(this.getID());
				...
				else if (this.m.IsTurnDone ||回合后sToGo == null)
				{
					tooltip.push({
						id = 4,
						type = "text",
				...
				else
				{
					tooltip.push({
						...
						type = "text",
						icon = "ui/icons/initiative.png",
						text = "行动于" + turnsToGo + (turnsToGo > 1 ? " turns" : " turn")
					});
				}
			}
	...
	function onDamageReceived( _attacker, _skill, _hitInfo )
	{
		...
		if (_hitInfo.DamageDirect < 1.0)
		{
			...
			if (this.m.BaseProperties.Armor[_hitInfo.BodyPart] != 0)
			{
				...
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "的armor被击中，受到 [b]" + this.Math.floor(_hitInfo.DamageArmor) + "[/b] 伤害");
			}
			...
			if (this.m.CurrentProperties.IsAffectedByInjuries && this.m.IsAbleToDie && damage >= this.Const.Combat.InjuryMinDamage && this.m.CurrentProperties.ThresholdToReceiveInjuryMult != 0 && _hitInfo.InjuryThresholdMult != 0 && _hitInfo.Injuries != null)
			{
				...
				while (potentialInjuries.len() != 0)
				{
					...
					if (injury.isValid(this))
					{
						...
						{
							this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "\'s " + this.Const.Strings.BodyPartName[_hitInfo.BodyPart] + " is hit for [b]" + this.Math.floor(damage) + "[/b] 伤害并遭受" + injury.getNameOnly() + "!");
						}
