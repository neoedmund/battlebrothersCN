this.player <- this.inherit("scripts/entity/tactical/human", {
	...
	function getTooltip( _targetedWithSkill = null )
	{
		...
		local回合后sToGo = this.Tactical.TurnSequenceBar.getTurnsUntilActive(this.getID());
		local tooltip = [
			{
				id = 1,
				type = "title",
		...
		tooltip.extend([
			{
				...
				text = this.Tactical.TurnSequenceBar.getActiveEntity() == this ? "现在行动！" : this.m.IsTurnDone ||回合后ToGo == null ? "回合结束" : "行动于" + turnsToGo + (turnsToGo > 1 ? " turns" : " turn")
			},
			{
				id = 3,
				type = "progressbar",
	...
	function getRosterTooltip()
	{
		...
		if (!this.isGuest())
		{
			...
			if (this.m.LifetimeStats.Battles != 0)
			{
				...
				{
					text = text + (" Took part in " + this.m.LifetimeStats.Battles + "场战斗");
				}
				...
				{
					text = text + (" and has " + this.m.LifetimeStats.Kills + "次击杀。");
				}
				else
				{
					text = text + ".";
	...
	function onDeath( _killer, _skill, _tile, _fatalityType )
	{
		...
		local isResurrectable = _fatalityType == this.Const.FatalityType.None || _fatalityType == this.Const.FatalityType.开膛;
		...
		if (!this.m.IsGuest && !this.Tactical.State.isScenarioMode() && _fatalityType != this.Const.FatalityType.Unconscious && (_skill != null && _killer != null || _fatalityType == this.Const.FatalityType.Devoured || _fatalityType == this.Const.FatalityType.Kraken))
		{
			...
			if (_fatalityType == this.Const.FatalityType.Devoured)
			{
				killedBy = "Devoured被a Nachzehrer";
			}
