this.faction_manager <- {
	...
	function getAlliedFactions( _f )
	{
		...
		{
			if (!this.m.Factions[_f].isTemporary敌人())
			{
	...
	function runSimulation()
	{
		...
		local barbarians = this.Const.DLC.Wildmen ? this.getFactionOfType(this.Const.FactionType.野蛮人) : null;
		local bandits = this.getFactionOfType(this.Const.FactionType.土匪);
		local nomads = this.Const.DLC.Desert ? this.getFactionOfType(this.Const.FactionType.OrientalBandits) : null;
		local orcs = this.getFactionOfType(this.Const.FactionType.兽人);
		local goblins = this.getFactionOfType(this.Const.FactionType.地精);
