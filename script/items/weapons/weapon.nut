	...
	function getTooltip()
	{
		...
		if (this.m.FatigueOnSkillUse > 0 && (this.getContainer() == null || !this.getContainer().getActor().getCurrentProperties().IsProficientWithHeavyWeapons))
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "武器技能积累 [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.FatigueOnSkillUse + "[/color] 更多疲劳值"
			});
		}
		else if (this.m.FatigueOnSkillUse < 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "Weapon skills build up [color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.FatigueOnSkillUse + "[/color] 更少疲劳值"
			});
		}
