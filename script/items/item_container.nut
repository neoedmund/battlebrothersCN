this.item_container <- {
	...
	function addToBag( _item, _slot = -1 )
	{
		...
		{
			this.logWarning("尝试添加物品 " + _item.getName() + "到背包, 但该物品已被放在其他地方");
			return false;
		}
		...
		if (_slot != -1)
		{
			...
			{
				this.logWarning("Attempted to add item " + _item.getName() + "到背包，该栏位不是空的或者被锁定");
				return false;
			}
	...
	function removeFromBag( _item )
	{
		...
		{
			this.logWarning("尝试从背包移除物品" + _item.getName() + "，但该物品已被放置到其他地方");
			return false;
		}
	...
	function removeFromBagSlot( _slot )
	{
		...
		if (item == null)
		{
			this.logWarning("尝试从空的背包插槽中移除物品");
			return false;
		}
	...
	function equip( _item )
	{
		...
		if (_item.getCurrentSlotType() != this.Const.ItemSlot.None)
		{
			this.logWarning("尝试装备物品" + _item.getName() + "，但该物品已被放在其他地方");
			return false;
		}
	...
	function unequip( _item )
	{
		...
		{
			this.logWarning("尝试卸下物品" + _item.getName() + "，但实际上未装备该物品");
			return false;
		}
	...
	{
		this.logInfo("##############################################");
		this.logInfo("# 展示库存" + this.getActor().getName());
