	...
	function getTooltip()
	{
		...
		else if (this.m.AdditionalAccuracy > 0)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Has [color=" + this.Const.UI.Color.PositiveValue + "]+" + this.m.AdditionalAccuracy + "%[/color] 额外命中，同时每格距离有 [color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color] 命中惩罚"
			});
		}
		else
		{
	...
	function onTargetHit( _skill, _targetEntity, _bodyPart, _damageInflictedHitpoints, _damageInflictedArmor )
	{
		if (_skill == this && _targetEntity.isAlive() && !_targetEntity.isDying())
		{
			...
			if (_bodyPart == this.Const.BodyPart.Head && !_targetEntity.getCurrentProperties().IsImmuneToDaze)
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + "击中了一击，造成" + this.Const.UI.getColorizedEntityName(_targetEntity) + "茫然了");
				}
			}
		}
	}
