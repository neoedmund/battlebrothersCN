this.ignite_firelance_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "点燃";
		this.m.Description = "点燃你的火矛的导火索，向你的对手释放火焰和烟雾。";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "有 [color=" + this.Const.UI.Color.PositiveValue + "]+40%[/color] 命中几率"
		});
		ret.push({
			id = 10,
			type = "text",
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "Has [color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color] 装弹剩余"
			});
		}
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]没有装弹剩余[/color]"
			});
		}
