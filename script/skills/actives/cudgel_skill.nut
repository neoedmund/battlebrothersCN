this.cudgel_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "重棒击";
		this.m.Description = "A slow overhead strike performed with full force to cudgel a target to pulp. Anyone hit with a blow like this will be茫然了 and gasping for air, unable to put full force behind their blows for two turns.";
	...
	function onUse( _user, _targetTile )
	{
		...
		if (success && target.isAlive() && !target.getCurrentProperties().IsImmuneToDaze)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + "一记重击使" + this.Const.UI.getColorizedEntityName(_targetTile.getEntity()) + " dazed");
			}
		}
