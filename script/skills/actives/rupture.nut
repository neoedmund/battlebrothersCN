this.rupture <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "撕裂";
		this.m.Description = "一种穿透攻击，可以覆盖2格的距离，可以从前线后方使用，超出大多数近战武器的范围。并且如果没有盔甲吸收打击则可以造成流血的伤口。";
	...
	function getTooltip()
	{
		...
		ret.extend([
			...
			},
			{
				...
				text = "如果没有盔甲吸收，则造成额外的 [color=" + this.Const.UI.Color.DamageValue + "]" + 10 + "[/color] 点持续的流血伤害"
			}
		]);
		ret.push({
			id = 6,
			...
			text = "有 [color=" + this.Const.UI.Color.PositiveValue + "]+5%[/color] 命中几率"
		});
