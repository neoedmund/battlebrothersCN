	...
	function onTeleportDone( _entity, _tag )
	{
		...
		foreach( actor in ZOC )
		{
			...
			if (actor.onAttackOfOpportunity(_entity, true))
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + "发起冲锋但被逼退了");
				}
		...
		if (potentialVictims.len() != 0)
		{
			...
			if (!victim.getCurrentProperties().IsImmuneToStun && this.Math.rand(1, 100) <= chance)
			{
				...
				if (_tag.OldTile.IsVisibleForPlayer || myTile.IsVisibleForPlayer)
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + "发起冲锋并击晕了" + this.Const.UI.getColorizedEntityName(victim));
				}
		...
		if (_tag.OldTile.IsVisibleForPlayer || myTile.IsVisibleForPlayer)
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + "发起冲锋");
		}
	}
