this.gore_skill <- this.inherit("scripts/skills/skill", {
	...
	function applyEffectToTarget( _user, _target, _targetTile )
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + "趔趄" + this.Const.UI.getColorizedEntityName(_target) + "持续 1 回合");
		}
		...
		if (!_target.getCurrentProperties().IsImmuneToKnockBackAndGrab && !_target.getCurrentProperties().IsRooted)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + "撞退了" + this.Const.UI.getColorizedEntityName(_target));
			}
	...
	function onTeleportDone( _entity, _tag )
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + "刺破(gores)");
		}
