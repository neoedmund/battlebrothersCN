this.skill <- {
	...
	function getDefaultTooltip()
	{
		...
		else if (this.m.DirectDamageMult > 0.0)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/regular_damage.png",
				text = "Inflicts [color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_min + "[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_max + "[/color] 生命值伤害，其中 [color=" + this.Const.UI.Color.DamageValue + "]0[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + damage_direct_max + "[/color] 可无视护甲"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/regular_damage.png",
				text = "Inflicts [color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_min + "[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_max + " 生命值伤害[/color]"
			});
		}
	...
	function use( _targetTile, _forFree = false )
	{
		...
		if (!_forFree)
		{
			this.logDebug(user.getName() + "使用skill " + this.getName());
		}
	...
	function getHitFactors( _targetTile )
	{
		...
		if (!this.m.IsRanged && targetEntity != null && targetEntity.get被包围Count() != 0)
		{
			ret.push({
				icon = "ui/tooltips/positive.png",
				text = "Surrounded"
		...
		if (!_user.isHiddenToPlayer() && !_targetEntity.isHiddenToPlayer())
		{
			...
			if (astray)
			{
				if (this.isUsingHitchance())
				{
					if (isHit)
					{
						this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + " uses " + this.getName() + " and the shot goes astray并命中" + this.Const.UI.getColorizedEntityName(_targetEntity) + "(几率:" + this.Math.min(95, this.Math.max(5, toHit)) + ", 掷出:" + rolled + ")");
					}
					else
					{
						this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + " uses " + this.getName() + " and the shot goes astray但没命中" + this.Const.UI.getColorizedEntityName(_targetEntity) + " (Chance: " + this.Math.min(95, this.Math.max(5, toHit)) + ", Rolled: " + rolled + ")");
					}
				}
				else
				{
					this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + " uses " + this.getName() + "，虽然偏离了目标，但打中了" + this.Const.UI.getColorizedEntityName(_targetEntity));
				}
			}
