this.tactical_state <- this.inherit("scripts/states/state", {
	...
	function onInitUI()
	{
		...
		tsb.setOnEntitySkill取消ClickedListener(this.turnsequencebar_onEntitySkillCancelClicked.bindenv(this));
		...
		ob.setOn退出PressedListener(this.topbar_options_onQuitButtonClicked.bindenv(this));
	...
	function onBattleEnded()
	{
		...
		if (isVictory)
		{
			...
			if (!this.isScenarioMode())
			{
				...
				this.World.Statistics.getFlags().set("LastEnemies失败edCount", this.m.MaxHostiles);
				...
				foreach( bro in playerRoster )
				{
					...
					else if (this.m.StrategicProperties.IsUsingSetPlayers && bro.isPlacedOnMap())
					{
						...
						{
							bro.improveMood(this.Const.MoodChange.BattleWon, "赢得了一场战斗");
						}
					}
			...
			if (!this.isScenarioMode())
			{
				...
				foreach( bro in playerRoster )
				{
					...
					else if (bro.getPlaceInFormation() <= 17)
					{
						...
						{
							bro.worsenMood(this.Const.MoodChange.BattleLost, "输掉了一场战斗");
						}
	...
	{
		this.logDebug("INFO: Next回合后 issued: " + _round);
	...
	function tactical_combat_result_screen_onQueryCombatInformation()
	{
		...
		case this.Const.Tactical.CombatResult.EnemyDestroyed:
			...
			result.subTitle = "敌人被消灭了, 在" + rounds + " round" + (rounds > 1 ? "s" : "");
			break;

		case this.Const.Tactical.CombatResult.EnemyRetreated:
			result.title = "Victory";
			result.subTitle = "敌人撤退了, 在" + rounds + " round" + (rounds > 1 ? "s" : "");
			break;

		case this.Const.Tactical.CombatResult.PlayerDestroyed:
			result.title = "Defeat";
			result.subTitle = "你战败了, 在" + rounds + " round" + (rounds > 1 ? "s" : "");
			break;

		case this.Const.Tactical.CombatResult.PlayerRetreated:
			result.title = "Retreat";
			result.subTitle = "你撤退了, 在" + rounds + " round" + (rounds > 1 ? "s" : "");
	...
	function showFleeScreen( _tag = null )
	{
		...
		this.Tooltip.hide();
		this.m.TacticalScreen.hide();
		this.m.TacticalDialogScreen.show("从战斗中撤退", "你确定吗？", this.Const.Strings.UI.FleeDialogueConsequences, "撤退！", "Cancel", this.tactical_flee_screen_onFleePressed.bindenv(this), this.tactical_flee_screen_onCancelPressed.bindenv(this));
		this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
		this.m.MenuStack.push(function ()
		{
			this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
